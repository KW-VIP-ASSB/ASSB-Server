...
from core.entity.style import Style,StyleImage,StyleMetadata,StylePrice,StyleCrawled,StyleRank
...
class LoadDataOperator(BaseOperator):
    ...
    def execute(self, context: Context):
        ...
        results = [ MusinsaSchema.model_validate(result) for result in self.styles ]
        ...
        style_ranks = [result.style_category_rank for result in results if result.style_category_rank is not None]
        for rank in style_ranks:
            rank['style_id'] = style_id_maps[rank.pop('style_idx')]
        with db.session() as session:
            stmt = pg_insert(StyleRank.__table__).values(style_ranks).on_conflict_do_nothing()
            session.execute(stmt)
            session.commit()
        ...
        with db.session() as session:
            stmt = pg_insert(Facet).values(brands_facets).on_conflict_do_nothing()
            session.execute(stmt)
            session.commit()

            stmt  =  sa.select(
                Facet.id, Facet.name
            ).where(Facet.name.in_([ facet['name'] for facet in brands_facets ])).where(Facet.type == "brand")
            results = session.execute(stmt).all()

            facet_name_id_map = { facet_name: facet_id for facet_id, facet_name in results }
            for brand_style_facet in brand_style_facets:
                brand_style_facet['facet_id'] = facet_name_id_map[brand_style_facet.pop('facet_idx')]
                brand_style_facet['style_id'] = style_id_maps[brand_style_facet.pop('style_idx')]
                for k,v in brand_style_facet.items():
                    assert v is not None, f"{k} is None"

            stmt = pg_insert(StyleFacet).values(brand_style_facets).on_conflict_do_nothing()
            session.execute(stmt)
            session.commit()
        ...
        self.log.info(f"Facet (Brand) Count: {len(brands_facets)}")
        self.log.info(f"Style Facet (Brand) Count: {len(brand_style_facets)}")
