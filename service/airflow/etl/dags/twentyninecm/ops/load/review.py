...
class ReviewLoadDataOperator(BaseOperator):
    ...
    def execute(self, context: Context):
        ...
        with db.session() as session:
            stmt = sa.select(Review.review_id,Review.id).where(Review.review_id.in_(review_ids)).where(Review.site_id == "qSh_hnGm49bnPArh")
        ...
        with db.session() as session:
            ...
            stmt = sa.select(Review.id, Review.review_id).where(Review.review_id.in_(review_ids)).where(Review.site_id == "qSh_hnGm49bnPArh")
            results = session.execute(stmt).all()
