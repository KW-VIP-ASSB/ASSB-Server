...
class FetchStyleReviewOperator(BaseOperator):
    url = "https://www.sivillage.com/goods/searchSivGoodsReview.siv"
    base_header =  {
        "User-Agent": (
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.review_per_page = review_per_page
    ...
    def execute(self, context: Context):
        ...
        transform_reviews = [self.transform(review, date=today) for review in reviews]
        self.log.info(f"reviews count: {len(reviews)}")
        self.log.info(f"review {reviews[0]}")
        return transform_reviews
    ...
    def get_total_review_count(self, html :str):
        ...
        if not soup.find("div", {"id": "review"}) :
            return None
        review_div = soup.find("div", {"id": "review"})
        return int(review_div["data-total_cnt"]) # type: ignore
    ...
    async def async_task_fetch_all_reviews(self, row):
        ...
        first_page=  await self.fetch_post(self.url, payload)
        total_page = self.get_total_review_count(first_page)  # type: ignore

        if total_page is None :
            return None
        total_page = min(int(total_page), self.max_page)
        ...
        for pagenum in range(1,total_page):
            ...
            **self.base_payload,
            ...
            tasks.append(self.fetch_post(self.url, payload))
    ...
    async def fetch_post(self, url: str,payload:dict, **kwargs)->str|None:
        ...
        self.aclient.headers = {
                **self.base_header,
                "Referer": f"https://www.sivillage.com/goods/initDetailGoods.siv?goods_no={payload["goods_no"]}",
            }
        ...
        try:
            ...
            retries +=1
            return await self.fetch_post(url, payload ,retries=retries)
