...
class TransformStyleResponseDataOperator(BaseOperator):
    ...
    def transform_data(self, style_info:dict, date:datetime)-> SivillageSchema|None:
        self.log.debug(f"Processing style info of type {type(style_info)}: {style_info}")
        try:
            parsed_data = SivillageSchema.build(product_info=style_info, ctg_gender_map=self.ctg_gender_map ,date= date )
            return parsed_data
