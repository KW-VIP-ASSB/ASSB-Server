...
class ReviewLoadDataOperator(BaseOperator):
    ...
    def execute(self, context: Context):
        ...
        if isinstance(self.reviews, XComArg):
            self.reviews = self.reviews.resolve(context) # type: ignore
        ...
        for review in self.reviews:
            review['recommended'] = True if review['recommended'] == 1 else False
            review['verifed_purchaser'] = True if review['verifed_purchaser'] == 1 else False
        ...
        with db.session() as session:
            stmt = sa.select(Review.review_id,Review.id).where(Review.review_id.in_(review_ids)).where(Review.site_id == "1PdU2BdfW5R2LXRM")
        ...
        with db.session() as session:
            ...
            stmt = sa.select(Review.id, Review.review_id).where(Review.review_id.in_(review_ids)).where(Review.site_id == "1PdU2BdfW5R2LXRM")
            results = session.execute(stmt).all()
