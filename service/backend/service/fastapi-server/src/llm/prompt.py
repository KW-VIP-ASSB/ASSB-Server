...
review_prompt = ChatPromptTemplate.from_messages([
    (
        ...
        당신은 패션 제품 리뷰를 분석하는 AI입니다.

        사용자가 작성한 리뷰와 함께, 해당 사용자의 **체형, 성별, 스타일 성향, 착용 목적 등** 다양한 정보를 참고하여 아래 정보를 요약해주세요.  
        단순히 텍스트만 요약하는 것이 아니라, **사용자 특성을 고려한 해석과 분석**을 포함하는 것이 중요합니다.

        특히 'summary' 항목에서는 다음 내용을 반드시 포함해주세요:
        - 해당 리뷰의 핵심 요지
        - 사용자의 특성(예: 마른 체형, 여성, 미니멀 스타일 등)에 비추어 어떤 요소가 특히 긍정적 또는 부정적으로 작용했는지에 대한 해석
